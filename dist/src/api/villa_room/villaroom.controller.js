"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.VillaRoomController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const dto_1 = require("./dto");
const villaroom_service_1 = require("./villaroom.service");
const guards_1 = require("../../middlewares/guards");
const common_2 = require("../../common");
let VillaRoomController = class VillaRoomController {
    constructor(villaRoomService) {
        this.villaRoomService = villaRoomService;
    }
    create(body) {
        return this.villaRoomService.create(body);
    }
    findAll(qr) {
        return this.villaRoomService.findAll(qr);
    }
    findOne(id) {
        return this.villaRoomService.findOne(id);
    }
    update(id, body) {
        return this.villaRoomService.update(id, body);
    }
    deleted(id) {
        return this.villaRoomService.delete(id);
    }
};
__decorate([
    (0, common_1.UseGuards)(guards_1.JwtSysGuard, guards_1.RoleGuard),
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [dto_1.CreateRoomDTO]),
    __metadata("design:returntype", void 0)
], VillaRoomController.prototype, "create", null);
__decorate([
    (0, common_1.Get)(),
    __param(0, (0, common_1.Query)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [dto_1.QueryRoomDTO]),
    __metadata("design:returntype", void 0)
], VillaRoomController.prototype, "findAll", null);
__decorate([
    (0, common_1.Get)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], VillaRoomController.prototype, "findOne", null);
__decorate([
    (0, common_1.UseGuards)(guards_1.JwtSysGuard, guards_1.RoleGuard),
    (0, common_1.Put)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, dto_1.UpdateRoomDTO]),
    __metadata("design:returntype", void 0)
], VillaRoomController.prototype, "update", null);
__decorate([
    (0, common_1.UseGuards)(guards_1.JwtSysGuard, guards_1.RoleGuard),
    (0, common_1.Delete)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", void 0)
], VillaRoomController.prototype, "deleted", null);
VillaRoomController = __decorate([
    (0, swagger_1.ApiTags)('villa_rooms'),
    (0, common_1.Controller)('villa_rooms'),
    (0, swagger_1.ApiSecurity)(common_2.name.API_KEY),
    __metadata("design:paramtypes", [villaroom_service_1.VillaRoomService])
], VillaRoomController);
exports.VillaRoomController = VillaRoomController;
//# sourceMappingURL=villaroom.controller.js.map